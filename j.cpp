...
// Function to run the Python script "left.py" or "right.py" based on the sign of the difference
void runPythonScript(int difference) {
    // If the difference is negative, run "python left.py 1000" (1 second)
    if (difference < 0) {
        system("python left.py 1000");
    } else {
        // If the difference is positive, run "python right.py"
        system("python right.py 1000");
    }
}
...
int main() {
    ...
    time_t last_decision_time = time(nullptr);
    ...
    while (true) {
        ...
        if (lowermost_red_x != -1 && lowermost_red_y != -1 && lowermost_green_x != -1 && lowermost_green_y != -1) {
            ...
            // Check if at least one second has passed since the last decision
            time_t now = time(nullptr);
            if (now - last_decision_time >= 1) {
                // Run the Python script if one second has passed since the last decision
                runPythonScript(x_diff);
                last_decision_time = now; // Update the last decision time
            }
        }
